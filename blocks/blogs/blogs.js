export default async function decorate(block) {
  ...
  try {
    ...
    let dateDropdown = ['latest to oldest', 'oldest to latest'];
    // Create and populate dropdowns
    const dropdown1 = createDropdown(mainContainer, 'date-dropdown', 'Select filters', dateDropdown);
    const dropdown2 = createDropdown(mainContainer, 'product_dropdown', 'Select category', originalData.map(item => item.category));
    ...
    function filterAndSortCards() {
      ...
      filteredData = filteredData.filter(entry => entry.description.toLowerCase().includes(searchText));
