...
function createControls(parent) {
    const controlsWrapper = createAndAppendElement(parent, 'div', { class: 'controls-wrapper' });

    const tabSelector = createAndAppendElement(controlsWrapper, 'div', { class: 'tab-selector' });
    const videoTab = createAndAppendElement(tabSelector, 'button', { class: 'tab active', 'data-tab-name': 'video' });
    videoTab.textContent = 'Video';
    const pictureTab = createAndAppendElement(tabSelector, 'button', { class: 'tab', 'data-tab-name': 'picture' });
    pictureTab.textContent = 'Picture Gallery';

    createAndAppendElement(controlsWrapper, 'input', { type: 'text', class: 'search-field', placeholder: 'What are you looking for?' });

    const dropdownSelector = createAndAppendElement(controlsWrapper, 'select', { class: 'dropdown-selector' });
    const optionDefault = createAndAppendElement(dropdownSelector, 'option', { value: '' });
    optionDefault.textContent = 'Sort By';
    const option1 = createAndAppendElement(dropdownSelector, 'option', { value: 'old_to_new' });
    option1.textContent = 'Date (Old to New)';
    const option2 = createAndAppendElement(dropdownSelector, 'option', { value: 'new_to_old' });
...
function createPictureCardsOnClick(data, container, fullData) {
    ...
    const categoryDiv = createAndAppendElement(container, 'div', { class: 'category-div' });

    const goBackButton = createAndAppendElement(categoryDiv, 'button', { class: 'go-back-button' });
    goBackButton.textContent = 'Go Back';

    const categoryHeading = createAndAppendElement(categoryDiv, 'h5', { class: 'category-heading' });
    ...
    const cardContainer = createAndAppendElement(container, 'div', { class: 'main-card-div', style: 'display: flex;' });
